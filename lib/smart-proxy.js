...
function handleRequest(chunk) {
	...
	if (cmd == REQUEST_CMD.CONNECT) {
		var that = this;
		this.request = chunk;
		this.proxy = net.connect(port, address, initProxy.bind(this));
		this.proxy.on('error',function(e){
			that.destroy();
		});
		this.proxy.on('connect',function(){
			that.proxy.middleware = {};
			that.proxy.middleware.functions = that.middleware_functions;
			that.proxy.middleware.external_machine = {};
			that.proxy.middleware.internal_machine = {};
			that.proxy.middleware.external_machine.host = address;
			that.proxy.middleware.external_machine.port = port;
			that.proxy.middleware.internal_machine.host = that.remoteAddress;
			that.proxy.middleware.internal_machine.port = that.remotePort;
			middleware.processMiddleware(that.proxy, null);		
		});
